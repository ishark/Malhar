...
/**
 * This is a Redis input operator, which scans all keys in Redis store
 * It converts Value stored as map to Plain Old Java Object. It outputs KeyValuePair with POJO as value
 * <p>
 * This output adapter Reads from RedisStore stored as <Key, Map>
 * It outputs a Key value pair <key, POJO> as tuples.
 * </p>
 *
...
public class RedisPOJOInputOperator extends
      ...
      for (int i = 0; i < dataColumns.size(); i++) {
        ...
        if (i < setters.size()) {
          ...
          case STRING:
            ...
            break;
          case BOOLEAN:
            ((SetterBoolean) setters.get(i)).set(mappedObject,
                Boolean.parseBoolean(value));
            break;
          case SHORT:
            ((SetterShort) setters.get(i)).set(mappedObject,
                Short.parseShort(value));
            break;
          case INTEGER:
            ((SetterInt) setters.get(i)).set(mappedObject,
                Integer.parseInt(value));
            break;
          case LONG:
            ((SetterLong) setters.get(i)).set(mappedObject,
                Long.parseLong(value));
            break;
          case FLOAT:
            ((SetterFloat) setters.get(i)).set(mappedObject,
                Float.parseFloat(value));
            break;
          case DOUBLE:
            ((SetterDouble) setters.get(i)).set(mappedObject,
  ...
  public void processFirstTuple(Map<String, String> value)
      throws ClassNotFoundException {
    objectClass = Class.forName(getOutputClass());

    System.out.println("In processFirstTuple... ");
    ...
    for (int i = 0; i < size; i++) {
      ...
      switch (type) {
      case STRING:
        setter = PojoUtils.createSetter(objectClass, getterExpression,
            String.class);
        break;
      case BOOLEAN:
  ...
  public void processTuples() {
    for (String key : keys) {
      if (store.getType(key).equals("hash")) {
        ...
        outputPort.emit(new KeyValPair<String, Object>(key,
            convertMapToObject(mapValue)));
      }
    }
